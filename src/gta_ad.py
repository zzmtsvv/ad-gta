...
from src.nn.attention_gta import TransformerBlock, TiedKVCache
from src.nn.memeff_rope_fn import RotaryEmbedding


class TransformerGTA(nn.Module):
    def __init__(
            self,
            seq_len: int = 40,
        ...
        # head_dim // 2 as in the original paper
        self.rope_dim = (hidden_dim // num_heads) // 2
...
class TedZadouriAD(nn.Module):
    ...
    def __init__(
        ...
        self.transformer = TransformerGTA(
            seq_len=seq_len,
    ...
    def forward(
            ...
            cache: TiedKVCache | None = None,
    ) -> tuple[torch.Tensor, TiedKVCache]:
